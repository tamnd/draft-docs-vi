...
`v-slot` có cú pháp rút gọn riêng là `#`, nên `<template v-slot:header>` có thể viết ngắn còn `<template #header>`. Hãy hiểu nó như "render đoạn template này vào slot 'header' của component con".
...
Bạn có thể dùng property [$slots](/api/component-instance.html#slots) kết hợp với [v-if](/guide/essentials/conditional.html#v-if) để làm điều đó.
...
[Đối số directive động](/guide/essentials/template-syntax.md#dynamic-arguments) cũng hoạt động với `v-slot`, cho phép định nghĩa tên slot động:
...
Lưu ý rằng biểu thức này vẫn phải tuân theo [các ràng buộc cú pháp](/guide/essentials/template-syntax.md#dynamic-argument-syntax-constraints) của đối số directive động.
...
Bạn có thể thắc mắc đâu là một trường hợp sử dụng tốt cho scoped slot. Đây là một ví dụ: hãy tưởng tượng một component `<FancyList>` render ra một danh sách item. Nó có thể bao bọc logic tải dữ liệu từ xa, dùng dữ liệu để hiển thị danh sách, thậm chí cả các tính năng nâng cao như phân trang hoặc infinite scrolling. Tuy nhiên, ta vẫn muốn nó linh hoạt về cách hiển thị từng item và giao việc định kiểu cho component cha đang dùng nó. Khi đó, cách dùng mong muốn có thể sẽ như sau:
